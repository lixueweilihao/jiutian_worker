 ...
 */
class ZookeeperLockCopyA implements Watcher {

    // 声明zk对象
    private ZooKeeper zk = null;
    // 此demo使用的集群，所以有多个ip和端口
    private static String CONNECT_SERVER = "10.3.7.232:2181,10.3.7.233:2181,10.3.6.20:2181";
    // session过期时间
    ...
    private boolean waitLock(String waitNode, int waidTime) throws KeeperException, InterruptedException {
        ...
        if (stat != null) {
            ...
            this.latch.await(waidTime, TimeUnit.HOURS);
            this.latch = null;
        }
        return true;
    }
    ...
    public static void main(String[] args) throws InterruptedException {
        ZookeeperLockCopyA zkLock = new ZookeeperLockCopyA();
        zkLock.lock();
        while (!wins()){
            Thread.sleep(50L);
        }
